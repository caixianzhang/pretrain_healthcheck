...
DYNAMIC_RETEST_SINGLE_NODE_CMD="cd ${PROJECT_REMOTE_DIR} && export PYTHONPATH=\"${PROJECT_REMOTE_DIR}\" && DIST_BACKEND=\"${DIST_BACKEND}\" DEVICE_VENDOR=\"${DEVICE_VENDOR}\" COMM_RUNTIME=\"${COMM_RUNTIME}\" ${DYNAMIC_RETEST_FAULT_ENV} torchrun --standalone --nproc-per-node=\"${GPUS_PER_NODE}\" -m pretrain_healthcheck.cli run-case-retest --output-dir \"\$HC_POD_RESULT_DIR/retest\" --plan-b64 \"${DYNAMIC_RETEST_ONLY_PLAN_B64:-\$HC_DYNAMIC_RETEST_PLAN_B64}\" --dtype \"${DTYPE}\" --ep-size \"${COLLECTIVE_BANDWIDTH_EP_SIZE}\" --warmup \"${COLLECTIVE_BANDWIDTH_WARMUP}\" --iters \"${COLLECTIVE_BANDWIDTH_ITERS}\" --diagnostic-iters 3 --seed \"${SEED}\" --group-id \"\$HC_JOB_NAME-\$HC_RUN_ID-retest\"; ${DYNAMIC_RETEST_COMPACT}"
DYNAMIC_RETEST_MULTI_NODE_CMD="cd ${PROJECT_REMOTE_DIR} && export PYTHONPATH=\"${PROJECT_REMOTE_DIR}\" && DIST_BACKEND=\"${DIST_BACKEND}\" DEVICE_VENDOR=\"${DEVICE_VENDOR}\" COMM_RUNTIME=\"${COMM_RUNTIME}\" ${DYNAMIC_RETEST_FAULT_ENV} torchrun --nnodes=\"\$WORLD_SIZE\" --nproc-per-node=\"${GPUS_PER_NODE}\" --node-rank=\"\$RANK\" --master-addr=\"\$MASTER_ADDR\" --master-port=\"__HC_MASTER_PORT__\" -m pretrain_healthcheck.cli run-case-retest --output-dir \"\$HC_POD_RESULT_DIR/retest\" --plan-b64 \"${DYNAMIC_RETEST_ONLY_PLAN_B64:-\$HC_DYNAMIC_RETEST_PLAN_B64}\" --dtype \"${DTYPE}\" --ep-size \"${COLLECTIVE_BANDWIDTH_EP_SIZE}\" --warmup \"${COLLECTIVE_BANDWIDTH_WARMUP}\" --iters \"${COLLECTIVE_BANDWIDTH_ITERS}\" --diagnostic-iters 3 --seed \"${SEED}\" --group-id \"\$HC_JOB_NAME-\$HC_RUN_ID-retest\"; ${DYNAMIC_RETEST_COMPACT}"
