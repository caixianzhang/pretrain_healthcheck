...
OUT_DIR="${OUT_DIR:-${HC_POD_RESULT_DIR:-${PROJECT_DIR}/results/metax_comm_path_$(date +%Y%m%d_%H%M%S)}}"
...
collect_env() {
  env | sort | grep -E '^(MCCL|NCCL|MACA|CUDA|TORCH|PYTORCH|MASTER_|WORLD_SIZE|RANK|LOCAL_|HC_|VC_|JOB_NAME|POD_IP)=' \
...
if [[ "${RUN_TORCH_DEBUG}" == "1" || "${RUN_TORCH_DEBUG}" == "true" ]]; then
  ...
  (
    ...
    export NCCL_DEBUG_SUBSYS="${NCCL_DEBUG_SUBSYS:-INIT,NET,GRAPH,ENV}"
    export MCCL_DEBUG="${MCCL_DEBUG:-INFO}"
    export MCCL_DEBUG_SUBSYS="${MCCL_DEBUG_SUBSYS:-INIT,NET,GRAPH,ENV}"
