...
if [[ -z "${MULTI_NODE_CMD:-}" ]]; then
  DYNAMIC_SUITE_COMPACT='rc=$?; python3 tools/dynamic_compact.py --input-dir "$HC_POD_RESULT_DIR" --kind dynamic-suite --stage "$HC_RUN_STAGE" --returncode "$rc" --run-id "$HC_RUN_ID" --pod-name "$HC_POD_NAME" --node-name "$HC_NODE_NAME" --pod-ip "$HC_POD_IP" --host-ip "$HC_HOST_IP"; exit "$rc"'
