...
DEFAULT_INJECT_ENV='export LD_LIBRARY_PATH="/usr/local/Ascend/driver/lib64:/usr/local/Ascend/driver/lib64/common:/usr/local/Ascend/driver/lib64/driver:/usr/local/Ascend/ascend-toolkit/latest/lib64:/usr/local/Ascend/cann-8.5.0/aarch64-linux/lib64:${LD_LIBRARY_PATH:-}"'
...
usage() {
  cat <<'EOF'
Usage:
  JOB_NAME=<vcjob-name> [env ...] bash scripts/ascend/run_vcctl_multi_node_batch_healthcheck_with_env_injection.sh
...
Common env:
  ...
  INJECT_ENV                 Shell snippet injected before DIST_BACKEND=.
                             Default: Ascend driver/toolkit library path.
